...
label_folder_path = osp.join(root, planet, dstype, 'label', subaux)
...
for filename in tqdm(os.listdir(data_folder_path)):
    ...
    sta_lta_ratios, sta_out, lta_out = stalta_classic(time, signal, fs, sta_factor=5, lta_factor=30)
    
    # Define a window size for the sliding window calculations (e.g., 100 samples)
    window_size = int(fs * 10)  # 10-second window as an example
    ...
    hjorth_params = sliding_window_hjorth_parameters(signal, window_size)
    ...
    # Add Hjorth parameters (activity, mobility, complexity) as separate columns
    hjorth_params.columns = ['activity', 'mobility', 'complexity']  # Rename Hjorth parameter columns
    feature_df = pd.concat([feature_df, hjorth_params], axis=1)
