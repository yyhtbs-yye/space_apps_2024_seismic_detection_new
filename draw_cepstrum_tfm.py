...
out_folder_path = osp.join(root, planet, dstype, 'cepstrum_analysis', subaux)
...
for filename in tqdm(os.listdir(in_folder_path)):
    ...
    log_spectrum = np.log(magnitude_spectrum + 1e-60)  # Adding small constant to avoid log(0)

    # Perform Inverse Fourier Transform on the log spectrum to get the cepstrum
    ...
    ax1.imshow(np.log(np.abs(cepstrum)), extent=[df['time_rel(sec)'].min(), df['time_rel(sec)'].max(), 0, window_size//2],
               cmap='viridis', vmin=-10, vmax=0, aspect='auto', origin='lower')
    ax1.set_ylabel('Quefrency (samples)')
    ax1.set_title(f'Cepstrum Analysis of Velocity Signal - {filename}')
