...
out_folder_path = osp.join(root, planet, dstype, 'wavelet_package_transform', subaux)
...
for filename in tqdm(os.listdir(in_folder_path)):
    ...
    # Get coefficients for each node in the wavelet packet tree
    nodes = [node.path for node in wp.get_level(wp.maxlevel, 'freq')]
    coefficients = np.array([wp[node].data for node in nodes])
    ...
    # First plot (Wavelet Packet Transform, part 1 of 2)
    ax1 = fig.add_subplot(gs[0, 0])
    ax1.imshow(np.log(np.abs(coefficients)+1e-60), extent=[df['time_rel(sec)'].min(), df['time_rel(sec)'].max(), 0, len(nodes)],
               cmap='viridis', vmax=-20, vmin=-25, aspect='auto', origin='lower')
    ax1.set_ylabel('Frequency band')
    ax1.set_title(f'Wavelet Packet Transform of Velocity Signal - {filename}')
